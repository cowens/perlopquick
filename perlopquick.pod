...
This is the heredoc operator.  It makes the lines after the current one up
to the next line that contains only X into an interpolating string.

X must be made of only a-z, A-Z, 0-9, or _.

To use other characters see L</E<lt>E<lt>E<quot>XE<quot>>.

=head3 Example

    # $s will hold "this is all part\nof the string\n"
    my $s = <<END_OF_STRING;
    this is all part
    of the string
    END_OF_STRING

    # prints 'debug:\n\t$s is [this is all part\nof the string\n]\nend debug\n'
    print "debug:\n", <<END_OF_STRING, "end debug\n";
    	\$s is [$s]
    END_OF_STRING

=head3 See Also

L<E<quot>XE<quot>>, L<E<lt>E<lt>E<quot>XE<quot>>, and
L<perlop/Quote and Quote-like Operators>
...
This is the double quote heredoc operator.  It makes the lines after the
current one up to the next line that contains only X into an interpolating
string.

X itself will be treated as a non-interpolating string, so the only escapes
that work are C<\"> and C<\\>.

=head3 Example

    # $x will hold "this is all part\nof the string\n"
    my $x = <<"END OF STRING";
    this is all part
    of the string
    END OF STRING
    ...
    # prints 'debug:\n\t$s is [this is all part\nof the string\n]\nend debug\n'
    print "debug:\n", <<"END OF STRING", "end debug\n";
    	\$x is [$x]
    END OF STRING

    # $y will hold "foo\nbar\n"
    my $y = <<"\'foo\\foo\tfoo\"$foo";
    foo
    bar
    \'foo\foo\tfoo"$foo
...
L<E<quot>XE<quot>>, L<E<lt>E<lt>X>, and
L<perlop/Quote and Quote-like Operators>
...
This is the single quote heredoc operator.  It makes the lines after the
current one up to the next line that contains only X into a non-interpolating
string.
...
L<perlop/Quote and Quote-like Operators>, and L<perlop/I/O Operators>

=head3 Description

This is the backtick operator. It runs X as an external command.

It is an interpolating string like C<""> that, instead of returning the
interpolated value, returns the value of running the interpolated value as an
external command.  In addition to Perl's interpolation, the string undergoes
shell expansion as well (eg * will become a list of the filenames in the
current directory).  Perl interpolation happens first, so to send a literal
dollar sign to the external command, you must escape it and the escape
character:

    my $stdout = `command \\\$`;

In list context, it will return the output broken into chunks using the input
record separator (C<$/>).

In scalar context, a single string containing the entire output of the
external command will be returned.
...
=head3 Example
    ...
    my $processes = `ps -ef`; # $processes contains the output of ps -ef

    # note the use of the escape character to prevent Perl's
    # interpolation of $USER.  This allows the shell to interpolate
    # the value of the $USER environment variable.

    local $/ = "\n"; # this is the default, but it pays to be careful

    # @matches contains one element for every line in the command's output
    my @matches = `grep \$USER /etc/group`;

=head3 See also

L<E<quot>XE<quot>>, L</qq(X)>, L</qx(X)>L<perlvar/$/>,
L<perlop/Quote and Quote-like Operators>,
...
This is the generalized backtick operator.  It runs X as an external command.

The delimiters C<()> are chosen by the user and may consist of either
bracketing characters (C<< qx<> >>, C<qx()>, C<qx{}>, and C<qx[]>) or matching
characters (C<qx##>, C<qxaa>, etc.).  It is generally used to allow the user
to avoid having to escape characters.

It is an interpolating string like C<qq()> that, instead of returning the
interpolated value, returns the value of running the interpolated value as an
external command.  In addition to Perl's interpolation, the string undergoes
shell expansion as well (eg * will become a list of the filenames in the
current directory).  Perl interpolation happens first, so to send a literal
...
=head3 Example
    ...
    my $processes = qx(ps -ef); # $processes contains the output of ps -ef
    ...
    # @matches contains one element for every line in the command's output
    my @matches = qx(grep \$USER /etc/group);

=head3 See also

L<E<quot>XE<quot>>, L</`X`>, L</qx(X)>L<perlvar/$/>,
L<perlop/Quote and Quote-like Operators>,
L<perlop/I/O Operators>
...
This is the regular expression (aka regex or regexp) compiler operator.  It
returns a compiled regular expression suitable for interpolation into the
match operator (C<m/X/>) or another regular expression compiler operator.

It takes all of the regular expression modifiers that the match and
substitution operators allow.

=head3 Example

    # regex to match an octet
    my $octet = qr(
       [0-9]       |
       [1-9][0-9]  |
       1[0-9][0-9] |
    ...
    my $ip_addr = qr( \b (?: $octet [.] ){3} $octet \b )x;

    # $match will contain 8.8.8.8
    my ($match) = "ip addr: 8.8.8.8" =~ /($ip_addr)/;

=head3 See Also

L</m/X/>, L<s/X/Y/>, and L<perlop/Regexp Quote-Like Operators>
...
This belongs to L<perlop/I/O Operators>.

=head3 Description

When the readline operator (<>) is empty, it is the magic readline operator
(also called the null filehandle readline operator).  It reads from C<STDIN> or
files passed as an argument to the program much in the same way classic Unix
filter programs (eg grep, sed, awk, etc.) do.

When first called in a program, if nothing is currently in C<@ARGV>, and the
C<ARGV> filehandle is not open, it will C<push> "-" onto @ARGV before
running.

If the C<ARGV> filehandle is open, it reads from that file until the input
record separator (C<$/>) is satisfied or the end of the file is reached and
returns the string it read.  Once it has reached the end of C<ARGV> it will
C<shift> the first element of <@ARGV> off and store it in C<$ARGV>.  It will
then open the file named by C<$ARGV> using the two argument version of C<open>
(this means the string "-", as added to C<@ARGV> above, will open STDIN, not
the file named "-") and store the resulting filehandle in C<ARGV>.  If there
are no more elements in C<@ARGV> when the current file ends, it will return
C<undef> (if in scalar context).

In list context, it returns a list of all of the remaining records based on
the contents to input record separator (<$/>).

In scalar context, it returns the next record based on the contents to input
record separator (<$/>) or C<undef> if there are no more records.

There is special magic for the magic readline operator when it is called from
inside of the test portion of a C<while> loop.  All of the following
statements are syntactically equivalent:

    while (<>) { ... }
    while ($_ = <>) { ... }
    while (defined($_ = <>)) { ... }

That is, when the magic readline operator is used in a while loop, it will
return its value to the default variable (C<$_>) unless you assign it a
different variable, and it will test for definedness rather than truthiness.

=head3 Security Warning

The magic readline operator uses the two argument version of open to open
files.  This means if one of your files is named "|command" or "command|",
then perl will attempt to run "command".  It also means if one of your files
is named ">file", it will attempt to open the file named "file", not ">file".
Likewise, there is no way to open a file named "-" since that will open STDIN
instead.

To fix this problem, use the CPAN module ARGV::readonly.  It cleans up the
contents of C<@ARGV> to prevent accidental code execution or opening of the
wrong files.

=head3 Example

    while (my $record = <>) {
        # $record contains data from either STDIN or
        # a file from @ARGV
    }

=head3 See also

L<perlvar/$/>, L<perlvar/ARGV>, L<perlvar/@ARGV>, L<perlvar/$ARGV>,
L<perlfunc/open>, L<perlfunc/readline>, and L<perlop/I/O Operators>
...
When what is between "<" and ">" is a bareword filehandle (eg C<STDIN>), a
lexical filehandle created with the C<open> function, or a simple scalar
variable containing the name of a bareword filehandle, typeglob (C<*STDIN>),
or a reference to a typeglob (C<\*STDIN>), it is the readline operator.

In list context, it returns the remaining contents of the file in
chunks as determined by the input record separator (C<$/>).

In scalar context, it returns the next record from the file as determined by
the input record separator (C<$/>) or C<undef> if there are no more records.

There is special magic for the readline operator when it is called from
inside of the test portion of a C<while> loop.  All of the following
statements are syntactically equivalent:
...
That is, when the readline operator is used in a while loop, it will
return its value to the default variable (C<$_>) unless you assign it a
different variable, and it will test for definedness rather than truthiness.
...
=head3 Example
    ...
    local $/ = "\n"; # this is the default, but best to be certain

    open my $fh, "<", $filename or die "could not open $filename: $!";

    while (my $record = <$fh>) {
        # $record contains the next record from /etc/passwd
    }

    fseek $fh, 0, 0; # rewind the file

    my $record1 = <$fh>; #reads the first record from /etc/passwd
    my $record2 = <$fh>; #reads the second record from /etc/passwd

=head3 See also

L<perlvar/$/>, L<perlfunc/open>, L<perlfunc/readline>, and
L<perlop/I/O Operators>
...
lexical filehandle created with the C<open> function, or a simple scalar
variable containing the name of a bareword filehandle, typeglob (C<*STDIN>),
or a reference to a typeglob (C<\*STDIN>), it is the glob operator.

In list context, the glob operator returns a list of filename expansions from
the GLOB expression such as the standard Unix shell (/bin/csh) would do.

In scalar context, it iterates over the such a list returning undef when there
are no more matching files.

There is special magic for the glob operator when it is called from
inside of the test portion of a C<while> loop.  All of the following
statements are syntactically equivalent:
...
That is, when the glob operator is used in a while loop, it will
return its value to the default variable (C<$_>) unless you assign it a
different variable, and it will test for definedness rather than truthiness.

=head3 Example

    # @files contains the list of files and directories in /etc.
    my @files = </etc/*>;
    ...
    while (my $record = <$fh>) {
        # $record contains data /etc/passwd
    }

=head3 See also

L<perlfunc/glob> and L<perlop/I/O Operators>
...
This is the binding operator.  It applies a regex match (C<m/X/>),
substitution (C<s/X/Y/>), or transliteration (C<tr/X/Y/> or C<y/X/Y/>) 
...
This is the negative binding operator.  It applies a regex match (C<m/X/>),
substitution (C<s/X/Y/>), or transliteration (C<tr/X/Y/> or C<y/X/Y/>)
PATTERN against a string variable.
...
L</X E<gt> Y>, L</X #E<gt>= Y>, and L</X ge Y>
