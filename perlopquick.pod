...
It reads from C<STDIN> or files passed as an argument to the program much in the
same way classic Unix filter programs (eg grep, sed, awk, etc.) do.
...
This is the readline operator (<>). It reads from the filehandle given to it
until the input record separator is satisfied or the end of the file is
reached and returns the string it read.

The filehandle may be a bareword filehandle (like STDIN) or a lexical
filehandle (ie a scalar variable) created by calling the C<open> function.
...
L<perlvar/$/>, L<perlfunc/open>, L<perlfunc/readline>, and L<perlop/I/O Operators>
