...
It creates a string literal out of a Perl variable.  If a scalar, array, or an index into
an array or hash is contained in the Perl Variable, then the value of that variable is inserted into
the string in place of the variable name.  Arrays are formated using C<$"> (the array separator, by default the space character):
...
L</qq(Perl Variable)>, L</'Perl Variable'>, L</q(Perl Variable)>, L<perlvar/$">, 
and L<perlop/Quote and Quote-like Operators>

=head2 qq(Perl Variable)
...
interpolating string operator).  It creates a string literal out of a Perl Variable.  If a
scalar, array, or an index into an array or hash is contained in the Perl Variable, then the value of
that variable is inserted into the string in place of the variable name.  Arrays
are formated using C<$">(the array separator, a space by default):
...
L</q(Perl Variable)>, L</'Perl Variable'> L<E<quot>Perl VariableE<quot>>, L<perlvar/$">, 
and L<perlop/Quote and Quote-like Operators>
...
It creates a string literal out of a Perl variable

To place a C<'> character inside the string, you must escape it with C<\>:

    my $quote = 'He said \'I like quotes.\'';

To place a C<\> character inside the string, you may escape it with another C<\>:
...
L<E<quot>Perl VariableE<quot>>, L</qq(Perl Variable)>, L</q(Perl Variable)>, L<perlvar/$">, 
and L<perlop/Quote and Quote-like Operators>

=head2 q(Perl Variable)
...
non-interpolating string operator).  It creates a string literal including the name of a Perl Variable.
...
L<E<quot>Perl VariableE<quot>>, L</'Perl Variable'>, L</qq(Perl Variable)>, L<perlvar/$">, 
and L<perlop/Quote and Quote-like Operators>
...
L</'Perl Variable'>, L</q(Perl Variable)>, and L<perlop/Quote and Quote-like Operators>
...
If X's sigil is C<@> instead, then an C<array slice> is created. 
The expression Y is placed in list context when it is evaluated and each 
item in the resulting list is then treated as a number; hence they are 
converted to a number before the list is used.
Each item will be turned into C<0> if it cannot be converted (and a warning
is thrown if L<warnings> are turned on).  The operator returns a list of items 
from the array X at the indexes in the list generated by Y.
  
Note: The elements of Y are not modified by the internal conversion to
numerical values.
...
=head3 Example
    ...
    my @b = @a[1, 2];    # @b is now ("b", "c"), an array slice of @a
...
at the indexes in the list generated by Y (an C<array slice> of the list X).
...
This is the infix array dereferencing operator (AKA the Arrow Operator).
It fetches the value at position Y in the array referenced by X. 
X is a simple scalar variable, an expression (an array element or a hash value), or a function returning a reference to an array.
When dealing with multiple levels of dereferencing (such as
...
hash-reference.
When dealing with multiple levels of dereferencing
...
=head3 Example
    ...
    my $r = $q--;    # $q is now 199 
                     # $r is "200 Quatloos for the newcomer"
...
C<=~> belongs to L<perlop/Binding Operators>.

=head3 Description

C<=~> is the binding operator.  It applies a regex match (C<m//>),
...
C<!~> is the negative binding operator.  
It behaves like the C<=~> operator, but the return is logically negated.
...
C<*> belongs to L<perlop/Multiplicative Operators>.

=head3 Description

C<*> is the multiplication operator; it returns the product of X multiplied by Y.
...
C</> is the division operator; it returns the quotient of X divided by Y.
